...
public class GestorArchivos {
	...
	public SistemaLogistico cargarDesdeArchivo(String rutaArchivo) throws ExcepcionLogistica {
		try {
			...
			if (!root.has("robopuertos") || !root.has("cofres") || !root.has("robots")) {
				throw new EstructuraInvalidaException("Faltan secciones obligatorias: robopuertos, cofres o robots",
						null);
			}
	...
	private List<RobotLogistico> cargarRobots(JsonNode array, List<Robopuerto> robopuertos, List<Cofre> cofres)
			throws ExcepcionLogistica {
		RobotFactory rf = new RobotFactory(robopuertos, cofres);
	...
	private void validarUbicaciones(List<Cofre> cofres, List<Robopuerto> rps) throws UbicacionDuplicadaException {
