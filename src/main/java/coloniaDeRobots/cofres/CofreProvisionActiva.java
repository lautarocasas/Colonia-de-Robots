...
public class CofreProvisionActiva extends Cofre {
    ...
    public void accionar(SistemaLogistico sistema) {
        ...
        List<Solicitud> solicitudes = sistema.obtenerSolicitudesPendientes(this);
        for (Solicitud s : solicitudes) {
            Item item = s.getItem();
            int pendiente = s.getCantidadPendiente();
            int disponible = getCantidadItem(item);
            if (pendiente <= 0 || disponible <= 0) continue;
            ...
            if (retirarItem(item, aEnviar)) {
                ...
                Logger.getLogger(getClass().getName()).info(() ->
                    String.format("[%s] Envío %d de %s a %s", ubicacion, aEnviar, item, s.getCofreDestino().getUbicacion())
                );
            }
        }
    }
